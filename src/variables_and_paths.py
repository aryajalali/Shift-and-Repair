...
def get_zeroshot_path(root, model):
    return Path(root, model, f"zeroshot.pt").as_posix()


def get_finetuned_path(root, dataset, model):
    print(root, dataset, model, "*" * 50)
    return Path(root, model, dataset, f"finetuned.pt").as_posix()
